...
public class SymbolTracer : MonoBehaviour
{
    ...
    private void CheckPath()
    {
        if (vRVectorProjection != null)
        {
            if (inputSource.GetNumberOfInputs() > 1)
            {
                List<Vector2> flatPathLeft = VRVectorProjection.ProjectVector3PathToVector2(tracedPathLeft, vRVectorProjection.mainCamera);
                List<Vector2> flatPathRight = VRVectorProjection.ProjectVector3PathToVector2(tracedPathRight, vRVectorProjection.mainCamera);
            }
            else if (inputSource.GetNumberOfInputs() == 1)
            {
